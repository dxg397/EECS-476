#include<PS4_traj_builder/PS4_traj_builder.h>
...
PS4TrajBuilder::PS4TrajBuilder()  {
    ...
    accel_max_ = default_accel_max; //0.45; //1m/sec^2
...
double PS4TrajBuilder::min_dang(double dang) {
...
double PS4TrajBuilder::sat(double x) {
    if (x > 1.0) {
        return 1.0;
    }
    if (x< -1.0) {
...
double PS4TrajBuilder::sgn(double x) {
    if (x > 0.0) {
        return 1.0;
    }
    if (x< 0.0) {
...
double PS4TrajBuilder::convertPlanarQuat2Psi(geometry_msgs::Quaternion quaternion) {
...
geometry_msgs::Quaternion PS4TrajBuilder::convertPlanarPsi2Quaternion(double psi) {
...
geometry_msgs::PoseStamped PS4TrajBuilder::xyPsi2PoseStamped(double x, double y, double psi) {
...
void PS4TrajBuilder::build_trapezoidal_spin_traj(geometry_msgs::PoseStamped start_pose,
    ...
    double ramp_up_dist = 0.45 * alpha_max_ * t_ramp*t_ramp;
    ...
    for (int i = 0; i < npts_ramp; i++) {
        ...
        //update orientation
        psi_des = psi_start + 0.45 * accel * t*t;
...
// profile is needed
void PS4TrajBuilder::build_spin_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    ...
    double ramp_up_dist = 0.45 * alpha_max_ * ramp_up_time*ramp_up_time;
...
void PS4TrajBuilder::build_travel_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    //decide if triangular or trapezoidal profile:
    double x_start = start_pose.pose.position.x;
    ...
    double ramp_up_dist = 0.45 * speed_max_ * speed_max_ / alpha_max_;
...
void PS4TrajBuilder::build_trapezoidal_travel_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    double x_start = start_pose.pose.position.x;
    double y_start = start_pose.pose.position.y;
    ...
    double ramp_up_dist = 0.45 * accel_max_ * t_ramp*t_ramp;
    ...
    for (int i = 0; i < npts_ramp; i++) {
        ...
        //update positions
        x_des = x_start + 0.45 * accel_max_ * t * t * cos(psi_des);
        y_des = y_start + 0.45 * accel_max_ * t * t * sin(psi_des);
...
void PS4TrajBuilder::build_triangular_travel_traj(geometry_msgs::PoseStamped start_pose, geometry_msgs::PoseStamped end_pose,std::vector<nav_msgs::Odometry> &vec_of_states) {
    double x_start = start_pose.pose.position.x;
    double y_start = start_pose.pose.position.y;
    double x_end = end_pose.pose.position.x;
    double y_end = end_pose.pose.position.y;
...
void PS4TrajBuilder::build_triangular_spin_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    nav_msgs::Odometry des_state;
    des_state.header = start_pose.header; //really, want to copy the frame_id
...
void PS4TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    //FINISH ME!
	double x_start = start_pose.pose.position.x;
	double y_start = start_pose.pose.position.y;

	double dx = 0.45; //stop in .5 meters
	double dy = 0.45; //stop in .5 meters
...
void PS4TrajBuilder::build_point_and_go_traj(geometry_msgs::PoseStamped start_pose,
        geometry_msgs::PoseStamped end_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
