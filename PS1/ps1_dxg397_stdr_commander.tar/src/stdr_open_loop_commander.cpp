...
int main(int argc, char **argv) {
    ...
    while(timer<3) { // move for 3 meters
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    timer=0.0; //reset the timer
    while(timer<5.6) { // move 5.6 meters
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    timer=0.0; //reset the timer
    while(timer<2.257) { //spin more than 68 degrees
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    twist_cmd.linear.x=speed; //command to move forward
    while(timer<4.7) { // move forward 4.7 meters
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
    ...
    twist_cmd.angular.z=0.0; // stop spinning in place
    twist_cmd.linear.x=speed; //command to move forward
    while(timer<4.5) { // move 4.5 meters
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
