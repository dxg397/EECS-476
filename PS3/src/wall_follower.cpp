...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) {
    ...
    for (int i = g_index_90deg_left; i >= g_index_90deg_right; i--) {
        r_test = laser_scan.ranges[i];
        //ROS_INFO("i, r_test = %d,  %f",i,r_test);
        //if(i==(abs(g_index_90deg_left) + abs(g_index_90deg_right))/2)
       
        if ( r_test <10) {
            g_range_max = r_test;
            g_index_max_dist_ping = i;
            
        }
    }
    int j=0;
     for (int i = (abs(g_index_90deg_left) + abs(g_index_90deg_right))/2 -2 ; i >= (abs(g_index_90deg_left) + abs(g_index_90deg_right))/2 +2; i++) {
         r_test = laser_scan.ranges[i];

   		 g_mid_dist[j] = r_test;
   		 j=j+1;
   		 ROS_INFO("%f",g_mid_dist[j]);
   		}
    ROS_INFO("min ping dist = %f at index %d", g_radius_min, g_index_min_dist_ping);
    ROS_INFO("max ping dist = %f at index %d, total number of pings = %d", g_range_max, g_index_max_dist_ping,abs(g_index_90deg_left) + abs(g_index_90deg_right)); 
...
int main(int argc, char **argv) {
    ...
    while (ros::ok()) {
        ...
        ros::spinOnce();
        if(check(g_mid_dist,6)  && g_index_min_dist_ping == g_index_tangent_left){
        	ROS_INFO("inside stright path");
 
        	timer = 0.0;
        	twist_cmd.linear.x = SPEED; //command to move forward
            twist_cmd.angular.z = 0.0;
            while(timer <  g_mid_dist[2]/SPEED)
            {
            	twist_commander.publish(twist_cmd);
            	timer+=SAMPLE_DT;
            	loop_timer.sleep();
